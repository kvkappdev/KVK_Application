...
import 'package:kvk_app/ui/viewmodels/edit/edit_reply_to_reply_viewmodel.dart';
import 'package:sliding_up_panel/sliding_up_panel.dart';
import 'package:stacked/stacked.dart';
import 'package:video_player/video_player.dart';

final log = getLogger("Edit Reply to Reply View");
final fileExtensionRegex = new RegExp(r"\.(?:.(?!\.))+$");

class EditReplyToReplyView extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    ...
    return ViewModelBuilder<EditReplyToReplyViewModel>.reactive(
      builder: (context, model, child) => WillPopScope(
        onWillPop: () {
          return model.onBackPressed(screenArguments: screenArguments);
        },
        child: Scaffold(
      ...
      viewModelBuilder: () => EditReplyToReplyViewModel(),
    );
  }
}

Widget attachedFiles(
    {@required EditReplyToReplyViewModel model,
...
Widget fileType(
    {@required EditReplyToReplyViewModel model, @required int index}) {
...
Widget bodyInput(
    {@required EditReplyToReplyViewModel model, @required double width}) {
  return Container(
...
Widget userDetails(
  ...
  return Container(
    ...
    child: Row(
      children: <Widget>[
        model.getProfilePic(userId: args.childReply.userId) != null
            ? CircleAvatar(
                backgroundColor: Colors.transparent,
                backgroundImage:
                    model.getProfilePic(userId: args.childReply.userId).image,
                radius: width * 0.1,
              )
            : CircleAvatar(
                backgroundColor: Colors.transparent,
        ...
        Container(
          ...
          child: Text(
            model.getName(userId: args.childReply.userId) != ""
                ? model.getName(userId: args.childReply.userId)
...
Widget topBar(
  ...
  return Container(
    ...
    child: Row(
      ...
      children: <Widget>[
        crossButton(width: width, model: model, args: args),
        Container(
          margin: new EdgeInsets.only(right: width * 0.3),
          child: Text(
            model.lang().editOptions[2],
            style: TextStyle(
              fontFamily: "Lato",
              fontSize: 20,
              height: 0.83,
...
Widget crossButton({
  ...
  return Container(
    child: RCrossButton(
      ...
      onPressed: () {
        model.reset();
        model.onBackPressed(screenArguments: args);
      },
    ),
  );
}
