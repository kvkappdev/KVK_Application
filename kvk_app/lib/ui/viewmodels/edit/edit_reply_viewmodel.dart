...
class EditReplyViewModel extends KVKViewModel {
  ...
  void loadArgs({@required ScreenArguments args}) async {
    replyText.text = args.reply.body;

    for (int i = 0; i < args.reply.imgs.length; i++) {
      AttachedImage img = AttachedImage();
      img.path = args.reply.imgs[i].path;
      img.local = false;
      img.size = args.reply.imgs[i].size;
    ...
    for (int i = 0; i < args.reply.vids.length; i++) {
      AttachedVideo vid = AttachedVideo();
      vid.path = args.reply.vids[i].path;
      vid.local = false;
      vid.size = args.reply.vids[i].size;
      _attachedVids.add(vid);
    }
    for (int i = 0; i < args.reply.files.length; i++) {
      AttachedFile file = AttachedFile();
      file.fileURL = args.reply.files[i].fileURL;
      file.local = false;
      file.fileSize = args.reply.files[i].fileSize;
      file.filetype = args.reply.files[i].filetype;
      file.name = args.reply.files[i].name;
